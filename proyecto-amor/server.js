...
// notificaciones push).
const PUSH_CONFIGURADO = Boolean(process.env.VAPID_PUBLIC_KEY && process.env.VAPID_PRIVATE_KEY);
if (PUSH_CONFIGURADO) {
...
async function enviarPush(autorQueOrigina, payload) {
    ...
    await Promise.all(r.rows.map(async (sub) => {
        if (sub.autor_tipo === autorQueOrigina.tipo && sub.autor_nombre === autorQueOrigina.nombre) return;
