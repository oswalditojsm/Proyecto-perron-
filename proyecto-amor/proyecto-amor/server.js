...
// Si no configuras las llaves VAPID en el .env, esta parte simplemente se
// desactiva sola (el resto del sitio sigue funcionando normal, solo sin
// notificaciones push).
let PUSH_CONFIGURADO = Boolean(process.env.VAPID_PUBLIC_KEY && process.env.VAPID_PRIVATE_KEY);
if (PUSH_CONFIGURADO) {
    try {
        webpush.setVapidDetails(
            process.env.VAPID_SUBJECT || "mailto:nadie@ejemplo.com",
            process.env.VAPID_PUBLIC_KEY,
            process.env.VAPID_PRIVATE_KEY
        );
        console.log("Notificaciones push: configuradas correctamente ✅");
    } catch (error) {
        // Ej. VAPID_SUBJECT mal formado (debe empezar con "mailto:" o "https://").
        // No tumbamos el servidor por esto: solo apagamos el push y lo dejamos bien claro en los logs.
        PUSH_CONFIGURADO = false;
        console.error("Notificaciones push DESACTIVADAS: revisa VAPID_SUBJECT/VAPID_PUBLIC_KEY/VAPID_PRIVATE_KEY en tu .env — error:", error.message);
    }
} else {
    console.log("Notificaciones push: no configuradas (faltan VAPID_PUBLIC_KEY / VAPID_PRIVATE_KEY en el .env)");
}

// Manda una notificación push a todo el mundo EXCEPTO a quien la generó (no
// tiene sentido notificarte a ti mismo de tu propio mensaje o cita). Si una
// suscripción ya no sirve (el navegador la invalidó), la borramos sola.
// Todo lo relevante se imprime en consola (logs de Render) para poder
// diagnosticar sin adivinar.
async function enviarPush(autorQueOrigina, payload) {
    if (!PUSH_CONFIGURADO) {
        console.log("[push] omitido: push no está configurado en el servidor");
        return;
    }

    const r = await db.execute("SELECT * FROM push_subscripciones");
    const destinatarios = r.rows.filter(
        (sub) => !(sub.autor_tipo === autorQueOrigina.tipo && sub.autor_nombre === autorQueOrigina.nombre)
    );
    console.log(`[push] "${payload.titulo}" — ${r.rows.length} suscripción(es) en total, ${destinatarios.length} destinatario(s) (excluyendo a ${autorQueOrigina.nombre})`);

    if (destinatarios.length === 0) {
        console.log("[push] nadie a quien notificar todavía (la otra persona no ha activado las notificaciones desde su celular)");
        return;
    }

    const paquete = JSON.stringify(payload);

    await Promise.all(destinatarios.map(async (sub) => {
        const suscripcion = {
            endpoint: sub.endpoint,
            keys: { p256dh: sub.p256dh, auth: sub.auth }
        };
        try {
            await webpush.sendNotification(suscripcion, paquete);
            console.log(`[push] enviado OK a ${sub.autor_nombre} (${sub.endpoint.slice(0, 60)}...)`);
        } catch (error) {
            console.error(`[push] FALLÓ para ${sub.autor_nombre} — código ${error.statusCode || "?"}: ${error.body || error.message}`);
            // 404/410 = el navegador ya no reconoce esa suscripción (se
            // desinstaló la app, se borraron datos, etc.) — la limpiamos.
            if (error.statusCode === 404 || error.statusCode === 410) {
                await db.execute({ sql: "DELETE FROM push_subscripciones WHERE id = ?", args: [sub.id] });
                console.log(`[push] suscripción vencida eliminada (${sub.autor_nombre})`);
            }
        }
    }));
}
...
app.get("/ubicacion.html", requiereAcceso, (req, res) => {
    res.sendFile(path.join(__dirname, "protegido", "ubicacion.html"));
});
...
app.post("/api/citas", requiereAcceso, async (req, res) => {
    ...
    enviarPush(autor, {
        ...
        cuerpo: detalle ? `${fecha} a las ${hora}: ${detalle}` : `${fecha} a las ${hora}`,
        url: "/cita.html"
    }).catch(() => {});
...
app.get("/api/push/vapid-public-key", requiereAcceso, (req, res) => {
    if (!PUSH_CONFIGURADO) return res.status(404).json({ error: "Push no configurado en el servidor" });
    res.json({ publicKey: process.env.VAPID_PUBLIC_KEY });
});

app.post("/api/push/suscribir", requiereAcceso, async (req, res) => {
    if (!PUSH_CONFIGURADO) return res.status(400).json({ error: "Push no configurado en el servidor" });
    ...
    const autor = await obtenerAutor(req);
    await db.execute({
        sql: `INSERT INTO push_subscripciones (endpoint, p256dh, auth, autor_tipo, autor_nombre) VALUES (?, ?, ?, ?, ?)
              ON CONFLICT(endpoint) DO UPDATE SET p256dh = excluded.p256dh, auth = excluded.auth, autor_tipo = excluded.autor_tipo, autor_nombre = excluded.autor_nombre`,
        args: [endpoint, keys.p256dh, keys.auth, autor.tipo, autor.nombre]
    });
    res.json({ ok: true });
});

app.post("/api/push/desuscribir", requiereAcceso, async (req, res) => {
    const { endpoint } = req.body || {};
    if (endpoint) await db.execute({ sql: "DELETE FROM push_subscripciones WHERE endpoint = ?", args: [endpoint] });
    res.json({ ok: true });
});

// Diagnóstico rápido, solo para ti (admin): revisa desde el navegador si el
// push está bien configurado y quién está suscrito, sin tener que buscar en
// los logs de Render. Entra a /api/push/estado estando logueado como admin.
app.get("/api/push/estado", requiereAdmin, async (req, res) => {
    const r = await db.execute("SELECT autor_tipo, autor_nombre, endpoint, creado_en FROM push_subscripciones");
    res.json({
        push_configurado_en_servidor: PUSH_CONFIGURADO,
        vapid_subject_usado: PUSH_CONFIGURADO ? (process.env.VAPID_SUBJECT || "mailto:nadie@ejemplo.com") : null,
        total_suscripciones: r.rows.length,
        suscripciones: r.rows.map((s) => ({
            quien: `${s.autor_nombre} (${s.autor_tipo})`,
            servicio_push: s.endpoint.includes("fcm.googleapis.com") ? "Google (Android/Chrome)" :
                            s.endpoint.includes("web.push.apple.com") ? "Apple (iPhone)" : "otro",
            desde: s.creado_en
        }))
    });
});

// ---- Rutas de UBICACIÓN EN VIVO ----
// Solo se guarda mientras la persona tiene la app abierta y activó
// explícitamente "compartir en vivo" — nunca corre en segundo plano ni sin
// que ella lo decida.
app.get("/api/ubicacion", requiereAcceso, async (req, res) => {
    const r = await db.execute("SELECT * FROM ubicaciones");
    res.json(r.rows);
});

app.post("/api/ubicacion", requiereAcceso, async (req, res) => {
    ...
    const autor = await obtenerAutor(req);
    await db.execute({
        sql: `INSERT INTO ubicaciones (autor_tipo, autor_nombre, lat, lng, precision_metros, compartiendo, actualizado_en)
              VALUES (?, ?, ?, ?, ?, 1, CURRENT_TIMESTAMP)
              ON CONFLICT(autor_tipo, autor_nombre) DO UPDATE SET
                lat = excluded.lat, lng = excluded.lng, precision_metros = excluded.precision_metros,
                compartiendo = 1, actualizado_en = CURRENT_TIMESTAMP`,
        args: [autor.tipo, autor.nombre, lat, lng, precision ?? null]
    });

    avisarCambio("ubicacion", { autor_tipo: autor.tipo, autor_nombre: autor.nombre, lat, lng, compartiendo: true });
    res.json({ ok: true });
});

app.post("/api/ubicacion/detener", requiereAcceso, async (req, res) => {
    const autor = await obtenerAutor(req);
    await db.execute({
        sql: "UPDATE ubicaciones SET compartiendo = 0 WHERE autor_tipo = ? AND autor_nombre = ?",
        args: [autor.tipo, autor.nombre]
    });
    avisarCambio("ubicacion", { autor_tipo: autor.tipo, autor_nombre: autor.nombre, compartiendo: false });
    res.json({ ok: true });
});
...
app.post("/api/chat/imagen", requiereAcceso, (req, res) => {
    ...
    upload.single("imagen")(req, res, async (err) => {
        ...
        try {
            ...
            enviarPush(autor, {
                titulo: `💬 ${autor.nombre}`,
                cuerpo: texto ? `📷 ${texto}` : "Te mandó una foto 📷",
                url: "/chat.html"
            }).catch(() => {});
...
async function iniciar() {
    ...
    await db.execute(`
        CREATE TABLE IF NOT EXISTS ubicaciones (
            ...
            compartiendo INTEGER NOT NULL DEFAULT 1,
            actualizado_en TEXT DEFAULT CURRENT_TIMESTAMP,
            PRIMARY KEY (autor_tipo, autor_nombre)
        )
    `);
